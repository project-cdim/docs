...
> [!NOTE] 
> ハードウェアを指す用語としてデバイスとリソースの２種類の表現が出てきます。デバイスはCPUやメモリ、ストレージなどのComposable Disaggregated Infrastructure (CDI) を構成する個々の物理ハードウェアのことを指し、リソースはデバイスの中で「ノードの構成要素」となる物理ハードウェアのことを指します。

> [!WARNING]
> 各リソースの使用率について注意事項が存在します。
> 詳細は[注意事項](#9-注意事項)を参照してください。
...
| 4 |構成反映状況|保留中の構成反映数を確認します。0の場合は問題はありません。|
| 5 |消費電力状況|1年間の消費電力量を確認します。初回の性能情報の更新時には、性能情報の蓄積のためにグラフが表示されるまで1時間ほど時間がかかります。|
...
- ログアウトする  
  ユーザボタンを選択することでログアウトできます。
  ![](imgs/home-user.png)
  ![](imgs/home-login.png)

<br>

##### 2. 各リソース種類ごとの消費電力とリソース状況を確認
画面左のメニューから「サマリー」を選択することでサマリー画面に移動します。
サマリー画面に表示されるダッシュボードから全体の消費電力やリソース状況、および各リソース種類毎の消費電力やリソース状況などを確認します。
画面の表示例は以下の通りです。
...
| 3 |リソース状況|現在使用しているリソースの数や使用しているリソースに異常がないかを確認します。Critical ResourceやWarning Resourceが0の場合はリソースは全て正常です。|
...
|番号|名前|説明|
|:--|:--|:--|
| 1 |ノード一覧|現在のノード状況を確認します。左からノードID、ノードに割り当てられているリソース数、使用できないリソース数、注意が出ているリソース数、警告が出ているリソース数、使用不可として除外したリソースドID|ノードIDをクリックすることでノード詳細画面に移動することが可能です。|
...
| 2 |リソース状況|現在使用しているリソースの数や使用しているリソースに異常がないかを確認します。Critical ResourceやWarning Resourceが0の場合はリソースは全て正常です。|
...
|番号|名前|説明|
|:--|:--|:--|
| 1 |デバイスID|リソース一つ一つを識別するIDです。初回起動時やリソースを追加した時に自動で割り振られます。|
...
##### 7. 構成反映状況の一覧を確認
画面左のメニューから「構成反映一覧」を選択することで構成反映一覧画面に移動します。
構成反映一覧画面ではこれまでに実行された構成反映一覧を確認します。構成反映の詳細は次章で説明します。
画面の表示例は以下の通りです。
...
|番号|名前|説明|
|:--|:--|:--|
| 1 |構成反映一覧|これまでの構成反映状況を確認します。左からApply ID、構成反映状況、構成反映の開始日時、構成反映の終了日時、ロールバック状況が表示されています。情報がない場合は何も記載されません。|
| 2 |構成反映詳細|Apply IDをクリックすることで構成反映詳細画面に移動することが可能です。|
| 3 |フィルター|各項目でフィルターを設定することが可能です。|
| 4 |昇順・降順|昇順表示、降順表示したい項目をクリックすることで昇順・降順表示できます。|
| 5 |ページ数|1ページに表示したいリソース数を変更できます。|
| 6 |リロード|即座に情報更新したい場合に使用します。|
...
##### 8. 構成反映状況の詳細を操作・確認
構成反映一覧画面でApply IDをクリックすることで構成反映詳細画面に移動します。
構成反映詳細画面では構成反映状況や構成反映で実行された手順を確認します。また、実行中の構成反映のキャンセルやロールバックなどの操作ができます。
画面の表示例は以下の通りです。
...
|番号|名前|説明|
|:--|:--|:--|
| 1 |構成反映操作|Apply IDと構成反映の状況を確認します。構成反映状態については下記の一覧を参照してください。|
| 2 |構成反映状況|構成反映の状況と開始、終了日時などを確認します。|
| 3 |移行手順|構成反映の詳細な手順を確認します。手順は構成反映が終了したときに表示され、構成反映が上手くいかなかったときはエラーメッセージを確認します。|

構成反映状態一覧  
- Complete : 構成反映を正常終了すると表示されます
- In_progress : 構成反映を実行中に表示されます
- Failed : 構成反映を異常終了すると表示されます
- Suspended : 構成案反映中に問題があり、一時中断した場合に表示されます
- Canceling,Canceled : 構成反映をキャンセルすると表示されます

実行できる内容は以下の通りです。
- キャンセルとロールバックの実行  
  構成反映を実行している場合はキャンセルかロールバックを実行できます。
