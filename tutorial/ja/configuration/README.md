...
> [!NOTE]
> ログの出力ディレクトリを変更後、そのコンポーネントが動かない場合はディレクトリを作成する必要があります。その場合はそれぞれのDockerfileの中にて、ログの出力ディレクトリを作成していますので、それもあわせて変更してください。
...
1. CDIMユーザー管理用のKeycloakのマスターURLにアクセスし、ログインする
   ...
   - Keycloak管理URLから操作する場合
    ...
    ログインした後に、左上の「Manage Realm」を押下し、CDIMを選択することで、CDIMの管理画面に切り替わります。

> [!NOTE]
> ユーザー管理画面からログインした場合、We are sorryの画面が出てくることがありますが、問題ありません。  
...
2. ユーザーを追加する  
    画面左のメニューにあるユーザーを押下し、「ユーザーの追加」ボタンを押します。
    ![](imgs/user-console-keycloak.png)
    ロケールの選択にて「English/Japanese」のいずれかから選択し、ユーザー名を入力します。
...
- ユーザーの権限を追加する
  1. 作成したユーザーの詳細画面を開く  
     ...
     画面左のユーザーを押し、権限を変更したいユーザーの詳細画面を開きます。  
     ![](imgs/user-details-keycloak.png)
     <br>
  2. 「ロールマッピング」から権限を変更する  
     * 権限を追加する場合  
     画面上部のタブから「ロールマッピング」を選択し、「ロールの割り当て」ボタンを押下します。現れたドロップダウンリストから、「レルムロール」を選択します。
    ![](imgs/user-rolemapping-keycloak.png)  
     追加したい権限をチェックし、割り当てを押します。
     ![](imgs/add-roles-keycloak.png)
     ロールを追加後、ログインし直すことで設定が適用されます。

   <details>
   <summary> CDIMの権限の詳細情報 </summary>
   「ロールの割り当て」ボタンを選択した後、「レルムロール」を押した場合に表示されます。
...
- 権限を解除する  
   「ロールマッピング」の画面で、削除したい権限にチェックを入れ、「割り当て解除」ボタンを押します。
...
1. ユーザーへのパスワードの割り当て  
   ...
   「クレデンシャル」タブを選択し、「パスワードを設定」を押します。パスワードを入力し終わったら「保存」ボタンを押します。
