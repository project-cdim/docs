...
デバイスの識別情報を格納した`OOBDeviceListItem`のリストです([表5-2. `get_device_info`の返り値](05a_Implementing_OOB_plugins_Appendix.md#表5-2-get_device_infoの返り値)参照)。  
異常時は下記の例外を送出します。

|No.|状況                                      |例外
|:-:|------------------------------------------|------------------------------------------------------------------------
| 1 |OOBで認証に失敗した場合                   |[`AuthenticationHWControlError`](07_Handling_Exceptions.md#ベース例外クラス)
| 2 |マネージャーがエラーを応答した場合        |[`ControlObjectHWControlError`](07_Handling_Exceptions.md#ベース例外クラス)
...
- `key_values`  
  OOBデバイスIDとデバイス種別を格納した辞書のリストです([表5-3. 引数`key_values`](05a_Implementing_OOB_plugins_Appendix.md#表5-3-引数key_values)を参照)。

### 5.2.2. 返り値

デバイスごとのスペック情報を格納した辞書です([表5-4. `get_spec_info`の返り値](05a_Implementing_OOB_plugins_Appendix.md#表5-4-get_spec_infoの返り値)を参照)。  
異常時は下記の例外を送出します。

|No.|状況                                      |例外
|:-:|------------------------------------------|------------------------------------------------------------------------
| 1 |指定されたデバイスのデバイス種別がプラグインのサポート対象外の場合 |[`RequestNotSupportedHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 2 |デバイスが存在しない場合                  |[`DeviceNotFoundHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 3 |OOBで認証に失敗した場合                   |[`AuthenticationHWControlError`](07_Handling_Exceptions.md#ベース例外クラス)
| 4 |マネージャーがエラーを応答した場合        |[`ControlObjectHWControlError`](07_Handling_Exceptions.md#ベース例外クラス)
...
プラグインの返り値とREST APIの項目名の対応は[表5-5. スペック情報とREST APIの項目名対応](05a_Implementing_OOB_plugins_Appendix.md#表5-5-スペック情報とrest-apiの項目名対応)を参照してください。
...
デバイスごとのメトリック情報を格納した辞書です([表5-6. `get_metric_info`の返り値](05a_Implementing_OOB_plugins_Appendix.md#表5-6-get_metric_infoの返り値)を参照)。  
異常時は下記の例外を送出します。
...
プラグインの返り値とREST APIの項目名の対応は[表5-7.メトリック情報とREST APIの項目名対応](05a_Implementing_OOB_plugins_Appendix.md#表5-7-メトリック情報とrest-apiの項目名対応)参照してください。
...
デバイスごとの電源状態を格納した辞書です([表5-8. `get_power_state`の返り値](05a_Implementing_OOB_plugins_Appendix.md#表5-8-get_power_stateの返り値)を参照)。  
異常時は下記の例外を送出します。
...
2. `OS起動確認`を実行するための設定を行います。  
   ※ `OS起動確認`はデバイスで稼働するエージェントにアクセスします。以下の手順でエージェントを[スタブ](03_Setup.md#31-スタブのセットアップ)に設定します。
...
実行結果を格納した辞書です([表5-9. 更新メソッドの返り値](05a_Implementing_OOB_plugins_Appendix.md#表5-9-更新メソッドの返り値)参照)。  
異常時は下記の例外を送出します。
...
| 1 |指定されたデバイスのデバイス種別がプラグインのサポート対象外の場合 |[`RequestNotSupportedHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 2 |デバイスが存在しない場合                  |[`DeviceNotFoundHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 3 |デバイスが電源操作不可の場合              |[`RequestNotSupportedHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 4 |電源ONに失敗した場合                      |[`PowerOnFailureHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 5 |OOBで認証に失敗した場合                   |[`AuthenticationHWControlError`](07_Handling_Exceptions.md#ベース例外クラス)
| 6 |マネージャーがエラーを応答した場合        |[`ControlObjectHWControlError`](07_Handling_Exceptions.md#ベース例外クラス)
...
| 2 |デバイスが存在しない場合                  |[`DeviceNotFoundHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 3 |デバイスが電源操作不可の場合              |[`RequestNotSupportedHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 4 |電源OFFに失敗した場合                     |[`PowerOffFailureHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 5 |OOBで認証に失敗した場合                   |[`AuthenticationHWControlError`](07_Handling_Exceptions.md#ベース例外クラス)
| 6 |マネージャーがエラーを応答した場合        |[`ControlObjectHWControlError`](07_Handling_Exceptions.md#ベース例外クラス)
...
| 2 |デバイスが存在しない場合                  |[`DeviceNotFoundHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 3 |デバイスが電源操作不可の場合              |[`RequestNotSupportedHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 4 |デバイスの電源がOFFの場合                 |[`RequestConflictHWControlError`](07_Handling_Exceptions.md#ベース例外クラス)
| 5 |CPUリセットに失敗した場合                 |[`CPUResetFailureHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 6 |OOBで認証に失敗した場合                   |[`AuthenticationHWControlError`](07_Handling_Exceptions.md#ベース例外クラス)
| 7 |マネージャーがエラーを応答した場合        |[`ControlObjectHWControlError`](07_Handling_Exceptions.md#ベース例外クラス)
...
| 2 |デバイスが存在しない場合                  |[`DeviceNotFoundHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 3 |デバイスが電源操作不可の場合              |[`RequestNotSupportedHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 4 |OSシャットダウンに失敗した場合            |[`PowerOffFailureHWControlError`](07_Handling_Exceptions.md#72-定義済み例外)
| 5 |OOBで認証に失敗した場合                   |[`AuthenticationHWControlError`](07_Handling_Exceptions.md#ベース例外クラス)
| 6 |マネージャーがエラーを応答した場合        |[`ControlObjectHWControlError`](07_Handling_Exceptions.md#ベース例外クラス)
